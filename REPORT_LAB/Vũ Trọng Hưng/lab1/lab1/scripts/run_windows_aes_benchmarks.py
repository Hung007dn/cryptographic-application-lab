...
EXE = BUILD_DIR / "encrypt_tool.exe"
...
def main():
    ...
    if not EXE.exists():
        raise FileNotFoundError(f"Cannot find encrypt_tool.exe: {EXE}")
    ...
    print("=" * 70)
    print("Lab 1 Windows AES Benchmark Runner")
    print("=" * 70)
    print(f"Root:      {ROOT}")
    ...
    for i in range(1, args.runs + 1):
        ...
        out_csv = RUNS_DIR / f"performance_report_windows_run_{i:02d}.csv"
        shutil.copy2(raw_csv, out_csv)

        print(f"[+] Saved: {out_csv}")

    print("\n[+] Windows benchmark complete.")
